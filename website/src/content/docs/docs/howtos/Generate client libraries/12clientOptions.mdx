...
```typespec
@clientOption(name: string, value: unknown | valueof unknown, scope?: string)
```
...
| `value`   | `string \| boolean \| number \| Model` | The value for the option. Supports primitives (string, boolean, number) or a reference to a TypeSpec model; the consuming emitter determines the expected shape. |
| `scope`   | `string` (optional)           | The target language scope. **Required** - omitting scope produces a `decorator-requires-scope` warning |
...
### TypeSpec Model Reference

You can also pass a TypeSpec model as the value. TCGC resolves the model into its `SdkType`, preserving any customizations (for example, `@alternateType`) so the consuming emitter can handle it:
