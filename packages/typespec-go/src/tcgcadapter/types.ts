...
export class TypeAdapter {
  ...
  readonly fieldsMap: Map<tcgc.SdkModelPropertyType, go.ModelField>;
  ...
  getPkg(): go.PackageContent {
    return this.codeModel.root.kind === "containingModule"
      ? this.codeModel.root.package
      : this.codeModel.root;
  }
  ...
  adaptTypes() {
    for (const enumType of this.ctx.sdkPackage.enums) {
      if ((enumType.usage & tcgc.UsageFlags.ApiVersionEnum) !== 0) {
        ...
        (enumType.usage & tcgc.UsageFlags.Input) === 0 &&
        (enumType.usage & tcgc.UsageFlags.Output) === 0
      ) {
    ...
    for (const modelType of this.ctx.sdkPackage.models) {
      if (modelType.name.length === 0) {
        ...
        modelType.access === "internal" &&
        (modelType.usage & tcgc.UsageFlags.Spread) !== 0
      ) {
        // we don't use the internal models for spread params
        continue;
      } else if (
        (modelType.usage & tcgc.UsageFlags.Input) === 0 &&
        (modelType.usage & tcgc.UsageFlags.Output) === 0
      ) {
        // skip types without input and output usage
        continue;
        ...
        helpers.isHttpFileType(modelType) ||
        (modelType.baseModel && helpers.isHttpFileType(modelType.baseModel))
      ) {
  ...
  private getPagedResponses(): Array<tcgc.SdkModelType> {
    ...
    const recursiveWalkClients = function (
      client: tcgc.SdkClientType<tcgc.SdkHttpOperation>,
    ): void {
      ...
      for (const sdkMethod of client.methods) {
        ...
        for (const httpResp of sdkMethod.operation.responses.values()) {
          ...
          if (
            !pagedResponses.find((each) => each.name === (<tcgc.SdkModelType>httpResp.type).name)
          ) {
  ...
  getWireType(
    ...
    switch (type.kind) {
      ...
      case "array": {
        ...
        // prefer elementTypeByValue. if false, then if the array elements have been explicitly marked asnullable then prefer that, else fall back to our usual algorithm
        const myElementTypeByValue = elementTypeByValue
          ? true
          : nullable
            ? false
        ...
        const elementType = this.getWireType(valueType, elementTypeByValue, substituteDiscriminator);
        ...
        arrayType = new go.Slice(
          ...
          this.getPtrType(elementType) : elementType
        );
      ...
      case "dict": {
        const valueType = this.getMapValueType(type.valueType, elementTypeByValue, substituteDiscriminator);
        const keyName = recursiveKeyName(
      ...
      case "duration": {
        switch (type.wireType.kind) {
          ...
          default:
            throw new AdapterError(
              "UnsupportedTsp",
              `unsupported duration wire format kind ${type.wireType.kind}`,
              type.wireType.__raw?.node,
            );
        }
      }
      case "model":
   ...
   */
  getMultipartContent(sliceOf: boolean, contentType?: string): go.MultipartContent | go.Slice<go.MultipartContent> {
  ...
  private getConstantType(enumType: tcgc.SdkEnumType): go.Constant {
    ...
    constType = new go.Constant(
      this.getPkg(),
      constTypeName,
      getPrimitiveType(enumType.valueType),
      `${accessPrefix}ossible${constTypeName}Values`,
    );
   ...
   */
  getLiteral<T extends go.LiteralType = go.LiteralType>(
    type: T,
    value: unknown,
    extensibleEnum: boolean = true,
  ): go.Literal<T> {
    ...
    switch (type.kind) {
      case "constant": {
        ...
        if (constantValue) {
          ...
          throw new AdapterError(
            "UnsupportedTsp",
            `invalid client side default ${<string>value} for fixed enum ${type.name}`,
          );
        }
        break;
      }
  ...
  private getInterfaceType(model: tcgc.SdkModelType): go.Interface {
    ...
    if (!helpers.isPolymorphicRoot(model)) {
      throw new AdapterError(
        "InternalError",
        `type ${model.name} isn't a discriminator root`,
        model.__raw?.node,
      );
    }
    ...
    if (!discriminatorField) {
      throw new AdapterError(
        "InternalError",
        `failed to find discriminator field for type ${model.name}`,
      );
    }
  ...
  private getModel(model: tcgc.SdkModelType): go.Model | go.PolymorphicModel {
    ...
    const annotations = new go.ModelAnnotations(
      omitSerde,
      (model.usage & tcgc.UsageFlags.MultipartFormData) !== 0,
    );
    if (helpers.isPolymorphicRoot(model) || model.discriminatorValue) {
      ...
      if (helpers.isPolymorphicRoot(model)) {
        ...
        if (!iface) {
          throw new AdapterError(
            "InternalError",
            `failed to find discriminator interface name for type ${model.name}`,
          );
        }
      }
  ...
  private getDiscriminatorLiteral(sdkProp: tcgc.SdkModelPropertyType): go.Literal {
    switch (sdkProp.type.kind) {
      ...
      default:
        throw new AdapterError(
          "UnsupportedTsp",
          `unsupported type kind ${sdkProp.type.kind} for discriminator property ${sdkProp.name}`,
          sdkProp.__raw?.node,
        );
    }
  }
  ...
  private getModelField(
    prop: tcgc.SdkModelPropertyType,
    modelType: tcgc.SdkModelType,
  ): go.ModelField {
  ...
  private getConstantValues(
    type: go.Constant,
    valueTypes: Array<tcgc.SdkEnumValueType>,
  ): Array<go.ConstantValue> {
    ...
    for (const valueType of valueTypes) {
      ...
      if (this.constValues.has(valueTypeName)) {
        ...
        throw new AdapterError(
          "NameCollision",
          `enum value ${valueType.name} was renamed to ${valueTypeName} which collides with an existing value`,
          valueType.__raw?.node,
        );
      }
  ...
  private getLiteralValue(constType: tcgc.SdkConstantType | tcgc.SdkEnumValueType): go.Literal {
    if (constType.kind === "enumvalue") {
      ...
      if (!constValue) {
        throw new AdapterError(
          "InternalError",
          `failed to find const value for ${constType.name} in enum ${constType.enumType.name}`,
          constType.__raw?.node,
        );
      }
    ...
    switch (constType.valueType.kind) {
      ...
      case "bytes": {
        ...
        literalByteArray = new go.Literal(
          this.adaptBytesType(constType.valueType),
          constType.value,
        );
      ...
      case "uint64": {
        ...
        literalInt = new go.Literal(
          new go.Scalar(constType.valueType.kind, false),
          constType.value,
        );
      ...
      default:
        throw new AdapterError(
          "UnsupportedTsp",
          `unsupported kind ${constType.valueType.kind} for LiteralValue`,
          constType.valueType.__raw?.node,
        );
    }
  ...
  private getUnionStruct(sdkUnion: tcgc.SdkUnionType, elementTypeByValue: boolean): go.UnionStruct {
    ...
    for (const variant of sdkUnion.variantTypes) {
      ...
      const fieldType = helpers.isPtrType(type)
        ? this.getPtrType(type)
        : type;
      goUnion.fields.push(
        new go.UnionField(
          recursiveVariantFieldName(fieldType),
          fieldType,
        ),
      );
    }
...
function getPrimitiveType(
  type: tcgc.SdkBuiltInType,
): "bool" | "float32" | "float64" | "int32" | "int64" | "string" {
  switch (type.kind) {
    case "boolean":
      return "bool";
    case "float32":
  ...
  switch (obj.kind) {
    ...
    case "plainTime":
      if (obj.encode !== "rfc3339") {
        throw new AdapterError(
          "UnsupportedTsp",
          `unsupported time encoding ${obj.encode}`,
          obj.__raw?.node,
        );
      }
...
function aggregateProperties(
  sdkContext: tcgc.SdkContext,
  model: tcgc.SdkModelType,
): { props: Array<tcgc.SdkModelPropertyType>; addlProps?: tcgc.SdkType } {
