...
export function generateServers(pkg: go.FakePackage, target: go.CodeModelType): ServerContent {
  ...
  for (const client of pkg.parent.clients) {
    ...
    for (const method of client.methods) {
      ...
      if (method.returns.result?.kind === "anyResult") {
        for (const httpStatus of getMethodStatusCodes(method)) {
          ...
          successCodes.push(
            `${helpers.formatStatusCode(httpStatus)} (returns ${go.getTypeDeclaration(result, pkg)})`,
          );
        }
    ...
    content += generateServerTransportClientDispatch(
      serverTransport,
      finalSubClients,
      imports,
      indent,
...
function generateServerTransportClientDispatch(
  serverTransport: string,
  subClients: Array<go.Client>,
...
function generateServerTransportMethodDispatch(
  serverTransport: string,
  client: go.Client,
  finalMethods: Array<go.MethodType>,
  indent: helpers.Indentation,
 ...
 */
function dispatchForOperationBody(
  pkg: go.FakePackage,
  receiverName: string,
  ...
  const allQueryParams = methodParamGroups.encodedQueryParams.concat(
    methodParamGroups.unencodedQueryParams,
  );
  if (
    allQueryParams.find(
      (each: go.QueryParameter) => each.location === "method" && !go.isLiteralParameter(each.style),
    )
  ) {
  ...
  if (bodyParam) {
    ...
    const emitCase = function (
      caseValue: string,
      paramVar: string,
      type: go.WireType,
      destIsByValue: boolean,
    ): string {
      ...
      if (isModelType(helpers.recursiveUnwrapMapSlice(type))) {
        ...
        switch (type.type) {
          ...
          default:
            throw new CodegenError(
              "InternalError",
              `unhandled multipart parameter primitive type ${type.type}`,
            );
        }
 ...
 */
function dispatchForPagerBody(
  pkg: go.FakePackage,
  receiverName: string,
  method: go.PageableMethod,
  imports: ImportManager,
  indent: helpers.Indentation,
): string {
  const operationName = helpers.fixUpMethodName(method);
 ...
 */
function parseHeaderPathQueryParams(
  pkg: go.FakePackage,
  method: go.MethodType,
  imports: ImportManager,
  indent: helpers.Indentation,
      ...
      if (elementType.kind !== "string") {
        ...
        switch (elementType.kind) {
          ...
          default:
            throw new CodegenError(
              "InternalError",
              `unhandled element kind ${elementType.kind}`,
            );
        }
        ...
        if (elementFormat === "bool") {
          ...
          elementFormat === "RFC1123" ||
          elementFormat === "RFC3339" ||
          elementFormat === "RFC7231" ||
          elementFormat === "Unix"
        ) {
          imports.add("time");
          fromVar = `parsed${naming.capitalize(elementFormat)}`;
  ...
  for (const param of helpers.getMethodParameters(method, consolidateHostParams)) {
    if (param.kind === "paramGroup") {
      if (param.groupName === method.optionalParamsGroup.groupName) {
        ...
        const countParams = param.params.filter(
