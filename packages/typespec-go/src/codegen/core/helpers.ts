...
export const doNotEditRegex =
  /^\/\/ Code generated .*( DO NOT EDIT\.$|[\s\S]*?^\/\/ DO NOT EDIT\.$)/m;
...
let overrideHeaderText: string | undefined;
...
export function contentPreamble(
  pkg: go.FakePackage | go.PackageContent | go.TestPackage,
  doNotEdit = true,
): string {
  ...
  if (doNotEdit) {
    ...
    text = text.replace(
      /^\/\/ Changes may cause incorrect behavior and will be lost if the code is regenerated\.$/m,
      "",
    );
  }
 ...
 */
export function formatParameterTypeName(
  scope: go.PackageType,
...
function sortParametersByRequired(
  ...
  let aRequired: boolean;
  let bRequired: boolean;
 ...
 */
export function sortClientParameters(
  params: Array<go.ClientParameter>,
  type: go.CodeModelType,
): Array<go.ClientParameter> {
    ...
    case "queryCollectionParam": {
      if (param.collectionFormat === "multi") {
        throw new CodegenError(
          "InternalError",
          "multi collection format should have been previously handled",
        );
      }
      ...
      switch (unwrappedElement.kind) {
        ...
        case "time": {
          ...
          const elemVal = unwrappedElement.utc
            ? `${param.name}[i].UTC()`
            : `${param.name}[i]`;
          return emitConvertOver(
            param.name,
...
export function formatCommentAsBulletItem(prefix: string, docs: go.Docs): string {
  ...
  const description = formatDocCommentWithPrefix(prefix, docs);
...
/** emits a * when needing to dereference */
...
export function getSerDeFormat(
  model: go.Model | go.PolymorphicModel,
    ...
    for (const method of client.methods) {
      for (const param of method.parameters) {
        if (
          ...
          (param.bodyFormat !== "JSON" && param.bodyFormat !== "XML")
        ) {
          continue;
        }
...
export function getAllClientParameters(
  pkg: go.PackageContent,
...
export function getCommonClientParameters(
  pkg: go.PackageContent,
  target: go.CodeModelType,
): Array<go.ClientParameter> {
