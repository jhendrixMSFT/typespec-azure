...
## Base Types (Experimental)

The `Azure.ResourceManager.BaseTypes` namespace (files `lib/base-types/base-types.tsp` and `lib/base-types/agent.tsp`) introduces ARM "base types" — schema contracts a resource declares conformance to via the `@azureBaseType(#{ baseType, version })` decorator (declared under `Azure.ResourceManager.BaseTypes`, so the reference anchor is `#@Azure.ResourceManager.BaseTypes.azureBaseType`). `BaseType` is an extensible enum (`Agent`, `Relationship`, `string`).

Key facts:

- The whole feature is **experimental**: any reference to a `BaseTypes` type emits the `basetypes-experimental` warning, which every using resource must suppress (`#suppress "@azure-tools/typespec-azure-resource-manager/basetypes-experimental" "..."`). `regen-docs` needs `--enable-experimental` (already in the package script).
- Agent base type lives in `Azure.ResourceManager.BaseTypes.Agents`. Resource templates: `Agent<Properties>` (TrackedResource, applies `@azureBaseType` for Agent automatically), `AgentConversation<Properties, AgentResource>` and `AgentResponse<Properties, AgentResource>` (both ProxyResource children).
- Two deployment models: **Appliance** (service-owned, all config read-only → `AgentPropertiesAppliance`/`AgentDefinitionAppliance`) and **Platform** (client-owned, writable → `AgentPropertiesPlatform`/`AgentDefinitionPlatform`). `baseTypes` is always ARM-managed/read-only in both.
- `AgentDefinition*<HasModelDeploymentRef = false, HasInstructions = false>`: the two booleans toggle optional `modelDeploymentRef` / `instructions` fields via the private `@baseTypeOptional(isPresent, isAppliance)` decorator.
- Response property mix-ins: `PreviousResponseProperty`, `ResponseOutputProperty`, `ResponseInstructionsProperty`.
- Two lint rules enforce structure: `arm-agent-base-type-child-resources` (Agent must have both a Conversation and a Response child) and `arm-agent-base-type-lifecycle-operations` (those children need full create/read/update/delete). Their `.md` rule descriptions live in `src/rules/`.
- Canonical sample: `packages/samples/specs/resource-manager/resource-types/agent/` (shows both deployment models). Referenced in docs via `<ExampleRef path="resource-manager/resource-types/agent" collapsed />`.
- How-to guide: `website/src/content/docs/docs/howtos/ARM/agent-base-type.mdx`.

## Website Rule Doc Directory

There is currently **no** `website/.../libraries/azure-resource-manager/rules/` directory in this repo; the auto-generated `reference/linter.md` links to `../rules/*.md` which do not exist. Do not create per-rule website pages for new rules — the linter.md table (regenerated by `pnpm regen-docs`) is the reference surface. Link how-to guides to `reference/linter.md`, not to non-existent per-rule pages.
