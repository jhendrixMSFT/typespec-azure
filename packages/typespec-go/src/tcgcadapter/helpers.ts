...
export function getClientOption<T extends boolean | string>(
  ...
  const clientOptions = src.decorators.filter(
...
export function hasDecorator(
  name: string,
  decorators: Array<tcgc.DecoratorInfo>,
...
export function isHttpFileType(model: tcgc.SdkModelType): boolean {
  ...
  return (
    model.namespace.toLowerCase() === "typespec.http" && model.name.toLowerCase().startsWith("file")
  );
}
