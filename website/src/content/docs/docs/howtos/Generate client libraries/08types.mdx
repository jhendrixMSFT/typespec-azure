...
:::note
The value type passed to `@clientDefaultValue` must match the property's scalar type. For example, passing a string value to a numeric property produces a `client-default-value-type-mismatch` warning. If `@alternateType` is present on the property, the value is validated against the alternate type instead.
:::
