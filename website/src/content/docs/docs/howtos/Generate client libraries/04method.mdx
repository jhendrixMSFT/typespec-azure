...
For fine tuning, use `@protocolAPI` and `@convenientAPI`. Each takes a boolean and a language scope. The scope must apply to Java, C#, or both; omitting it or using a scope that excludes both languages produces a `decorator-requires-scope` warning.
